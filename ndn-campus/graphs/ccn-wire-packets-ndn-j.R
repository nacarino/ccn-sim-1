...
rawdata.flood = read.table ("/home/jelfn/git/ccn-sim-1/ndn-campus/results/CCNWireless-rate-trace-flood-matome.txt", header=T)
rawdata.bestr = read.table ("/home/jelfn/git/ccn-sim-1/ndn-campus/results/CCNWireless-rate-trace-bestr-matome.txt", header=T)
rawdata.smart = read.table ("/home/jelfn/git/ccn-sim-1/ndn-campus/results/CCNWireless-rate-trace-smart-matome.txt", header=T)
...
rawdata.flood = subset (rawdata.flood, Time < 26 )
rawdata.bestr = subset (rawdata.bestr, Time < 26 )
rawdata.smart = subset (rawdata.smart, Time < 26 )
...
inlist = list(flood.data.in, bestr.data.in, smart.data.in)
outlist = list(flood.data.out, bestr.data.out, smart.data.out)
intinlist = list(flood.int.in, bestr.int.in, smart.int.in)
intoutlist = list(flood.int.out, bestr.int.out, smart.int.out)
satsatlist = list(flood.sat.sat, bestr.sat.sat, smart.sat.sat)
satoutlist = list(flood.sat.out, bestr.sat.out, smart.sat.out)
...
g.datain2 = ggplot (data=allindata, aes (x=Time, y=PacketRaw.sum, linetype=Variable, colour=Variable, shape=Variable)) +
  ...
  scale_linetype_discrete(name = "Strategy Types",
                          labels = c("BestRoute", "Flooding", "SmartFlooding")) +
...
combinedlist = list(flood.combined, bestr.combined, smart.combined)
